...
- при отдельном repo-level opt-in может сам создать и переключить локальную ветку, если это явно безопаснее
...
После этого он должен отдельно спросить, нужно ли разрешить автоматическое локальное переключение веток в этом repo. Если пользователь соглашается и на это, `branch-pilot` позже сможет сам создать и переключить рекомендованную локальную ветку, когда решение однозначное и worktree безопасен.
...
Если для repo включён opt-in на автоматическое локальное переключение веток, он может сам создать и переключить рекомендованную локальную ветку без повторного вопроса каждый раз. Remote он при этом не трогает.

## Чего Он Не Делает Автоматически

- по умолчанию не создаёт и не переключает ветки
- не коммитит
- не пушит
- не мержит
