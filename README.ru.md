...
- разговаривает на языке пользователя и может полностью вести branch-подсказки по-русски
...
Если он рекомендует новую ветку, но не переключает её сам, он должен коротко объяснить почему: opt-in выключен, worktree смешан или текущее git-состояние небезопасно для переключения.
